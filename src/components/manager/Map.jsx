      ...
      <div className="absolute right-[110%] top-0 w-44 md:w-56 bg-slate-900/95 backdrop-blur-md text-white p-2 md:p-4 rounded-xl shadow-2xl border border-slate-700 pointer-events-auto z-[60] animate-fade-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-1 md:mt-3 space-y-1 md:space-y-2 mb-2 md:mb-4">
          <div className="flex justify-between items-center text-[9px] md:text-xs">
            ...
            {unpinnedData.status === 'critical' ? (<span className="font-bold text-red-400 flex items-center gap-1"><i className="ph ph-siren animate-pulse"></i> {modeType === 'planned_maintenance' ? 'ZALEGŁOÄąĹˇCI' : 'AWARIA KRYTYCZNA'}</span>) : unpinnedData.status === 'in_progress' ? (<span className="font-bold text-blue-400 flex items-center gap-1"><i className="ph ph-wrench animate-pulse"></i> Serwis w trakcie</span>) : unpinnedData.status === 'warning' ? (<span className="font-bold text-amber-400">{modeType === 'planned_maintenance' ? 'Zbliża się serwis' : 'Usterka'}</span>) : (<span className="font-bold text-emerald-400">{modeType === 'planned_maintenance' ? 'Brak pilnych' : 'Gotowe'}</span>)}
          </div>
                      ...
                      {isHovered && mode === 'view' && (
                        <div className={`absolute ${tooltipPosClass} w-44 md:w-56 bg-slate-900/95 backdrop-blur-md text-white p-2 md:p-4 rounded-xl shadow-2xl border border-slate-700 pointer-events-auto z-50 animate-fade-in`} onMouseEnter={() => handleMouseEnter(pin.id)} onMouseLeave={handleMouseLeave}>
                          ...
                          <div className="mt-1 md:mt-3 space-y-1 md:space-y-2 mb-2 md:mb-4">
                            <div className="flex justify-between items-center text-[9px] md:text-xs">
                              <span className="text-slate-400">Stan:</span>
                              {pin.status === 'critical' ? (<span className="font-bold text-red-400 flex items-center gap-1"><i className="ph ph-siren animate-pulse"></i> {modeType === 'planned_maintenance' ? 'ZALEGŁOÄąĹˇCI' : 'AWARIA KRYTYCZNA'}</span>) : pin.status === 'in_progress' ? (<span className="font-bold text-blue-400 flex items-center gap-1"><i className="ph ph-wrench animate-pulse"></i> Serwis w trakcie</span>) : pin.status === 'warning' ? (<span className="font-bold text-amber-400">{modeType === 'planned_maintenance' ? 'Zbliża się serwis' : 'Usterka'}</span>) : (<span className="font-bold text-emerald-400">{modeType === 'planned_maintenance' ? 'Brak pilnych' : 'Gotowe'}</span>)}
                            </div>
          ...
          {mode === 'view' && modeType === 'tickets' && (
            <div className="absolute top-4 right-2 lg:top-4 lg:right-4 z-[110] bg-slate-900/90 backdrop-blur-md p-2 lg:px-5 lg:py-4 rounded-lg lg:rounded-xl border border-slate-700 shadow-xl text-white pointer-events-none flex flex-col gap-1 lg:gap-3 min-w-[140px] lg:min-w-[220px] scale-[0.65] lg:scale-100 origin-top-right">
              ...
              <div className="flex items-center justify-between gap-4 relative pointer-events-auto cursor-pointer group rounded p-1 -mx-1 hover:bg-slate-800/50 transition-colors" onMouseEnter={(e) => handleMouseEnter('unpinned_items', e)} onMouseLeave={handleMouseLeave} onClick={() => setHoveredPin(hoveredPin === 'unpinned_items' ? null : 'unpinned_items')}>
                ...
                <span className={`font-bold text-lg ${getUnpinnedBgColor()}`}>{currentSubmapId ? machines.filter(m => m.regionId === currentSubmapId && m.xPercent == null).length : machines.filter(m => m.xPercent == null && !m.regionId).length}</span>
